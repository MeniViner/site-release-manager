...
export function unwrapODataRecord(payload) {
...
export function unwrapODataCollection(payload) {
...
export function classifyFolderProbe({
  status,
  payload,
  expectedPath,
  libraryRoot = false,
...
/**
 * Classify the combined evidence for a child folder. Readiness is deliberately
 * conjunctive: list item, folder object, owning list, exact path and the real
 * parent's child enumeration must all identify the same object.
 */
export function classifyFolderReadiness({
  expectedPath,
  expectedParentPath,
  expectedLibraryId,
  listItem,
...
export function createSharePointClient(options = {}) {
  ...
  /**
   * Compatibility REST creator. Production uses the library-bound JSOM adapter;
   * this path performs one mutation only and is accepted only after the same
   * strict postcondition probe succeeds.
   */
