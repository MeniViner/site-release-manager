...
const { SP_ERROR, classifySharePointError, sharePointError } = require("./sharepointErrors.js");
const { stabilize, retryOperation, STABILIZE_RETRY, DEFAULT_RETRY, RetryBudgetExceededError } = require("./retry.js");
const { SEED_CONTENT_TYPE, ASSET_CONTENT_TYPE, normalizePath } = require("./sharepointClient.js");
const { DEPLOYMENT_METADATA_FILE, RUNTIME_CONFIG_FILE, RUNTIME_BOOTSTRAP_FILE, parseIndexReferencesFromHtml } = require("./universalManifest.js");
const { RUNTIME_BOOTSTRAP_MARKER, RUNTIME_BOOTSTRAP_GLOBAL, parseRuntimeBootstrapConfig } = require("./runtimeBootstrap.js");
const LIBRARY_OUTCOME = Object.freeze({
  REUSED: 'REUSED_EXISTING_EXACT_LIBRARY',
  CREATED: 'CREATED_EXACT_LIBRARY',
  RECOVERED: 'RECOVERED_AFTER_CREATE_ERROR',
});

const PROVISIONING_ERROR = Object.freeze({
...
const BACKUP_OUTCOME = Object.freeze({
  PASSED: 'PASSED',
  PARTIAL: 'PARTIAL',
  FAILED: 'FAILED',
...
function isFatalProvisioningError(error) {
 ...
 */
async function ensureExactLibrary(client, spec, options = {}) {
 ...
 */
async function ensureFolderTree(client, folderPaths, options = {}) {
...
function orderParentFirst(folderPaths) {
 ...
 */
async function ensureTxtSeeds(client, seeds, options = {}) {
 ...
 */
async function createTxtBackup(client, options = {}) {
  const {
    sourceFiles = [],
    siteAssetsRoot,
    host,
 ...
 */
async function uploadReleaseAssets(client, plan, options = {}) {
  const {
...
async function verifyRemoteFile(client, targetPath, expected, options = {}) {
 ...
 */
async function verifyFinalRuntimeConfig(client, fetchImpl, verification, options = {}) {
 ...
 */
async function finalAppSmoke(client, distRoot, options = {}) {
  const {
...
module.exports = {
  ...
  orderParentFirst: orderParentFirst,
  ensureTxtSeeds: ensureTxtSeeds,
  createTxtBackup: createTxtBackup,
  uploadReleaseAssets: uploadReleaseAssets,
  verifyRemoteFile: verifyRemoteFile,
  verifyFinalRuntimeConfig: verifyFinalRuntimeConfig,
  finalAppSmoke: finalAppSmoke,
