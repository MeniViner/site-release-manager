...
const { RUNTIME_BOOTSTRAP_FILE } = require("./universalManifest.js");
/** The global Site Builder's loadEmbeddedRuntimeConfig() reads first. */
const RUNTIME_BOOTSTRAP_GLOBAL = 'SITE_BUILDER_RUNTIME_CONFIG';
/** The legacy alias the same loader accepts. Both are populated. */
const RUNTIME_BOOTSTRAP_LEGACY_GLOBAL = '__SITE_BUILDER_RUNTIME_CONFIG__';
 ...
 */
const RUNTIME_BOOTSTRAP_MARKER = `window.${RUNTIME_BOOTSTRAP_GLOBAL}`;

/** The exact tag injected into the staged index.html. */
const RUNTIME_BOOTSTRAP_SCRIPT_TAG = `<script src="./${RUNTIME_BOOTSTRAP_FILE}"></script>`;
 ...
 */
function buildRuntimeBootstrapSource(runtimeConfig) {
 ...
 */
function parseRuntimeBootstrapConfig(source) {
...
function hasRuntimeBootstrapReference(html) {
...
function countRuntimeBootstrapReferences(html) {
 ...
 */
function findFirstModuleScriptIndex(html) {
 ...
 */
function findFirstForeignScriptIndex(html) {
  const text = maskHtmlComments(html);
  const pattern = new RegExp(SCRIPT_OPEN_TAG_PATTERN.source, 'gi');
  let match = pattern.exec(text);
  while (match) {
...
function findRuntimeBootstrapIndex(html) {
 ...
 */
function injectRuntimeBootstrapIntoIndexHtml(html) {
...
module.exports = {
  buildRuntimeBootstrapSource: buildRuntimeBootstrapSource,
  parseRuntimeBootstrapConfig: parseRuntimeBootstrapConfig,
  hasRuntimeBootstrapReference: hasRuntimeBootstrapReference,
  countRuntimeBootstrapReferences: countRuntimeBootstrapReferences,
  findFirstModuleScriptIndex: findFirstModuleScriptIndex,
  findFirstForeignScriptIndex: findFirstForeignScriptIndex,
  findRuntimeBootstrapIndex: findRuntimeBootstrapIndex,
  injectRuntimeBootstrapIntoIndexHtml: injectRuntimeBootstrapIntoIndexHtml,
  RUNTIME_BOOTSTRAP_GLOBAL: RUNTIME_BOOTSTRAP_GLOBAL,
  RUNTIME_BOOTSTRAP_LEGACY_GLOBAL: RUNTIME_BOOTSTRAP_LEGACY_GLOBAL,
  RUNTIME_BOOTSTRAP_MARKER: RUNTIME_BOOTSTRAP_MARKER,
  RUNTIME_BOOTSTRAP_SCRIPT_TAG: RUNTIME_BOOTSTRAP_SCRIPT_TAG,
};
